 ...
 *
 * Provides common helpers for object manipulation and validation, such as deep
 * cloning, deep equality checks, and freezing objects.
 *
 * Design:
...
export const ObjectUtils = {
  ...
  /**
   * Set a value at a dot-notation path, creating objects as needed. Mutates
   * input.
   *
   * @template T - Object type
   ...
   * - When the source has a key with value `null`, that key is deleted from the
   *   result (pruned)
   *
   ...
   * @returns A new object that is the result of merging `target` and `source`
   *
   * @example
   * ```typescript
   * ObjectUtils.deepMerge(
   *   { a: 1, b: { c: 2, x: 1 }, arr: [1,2] },
   *   { b: { d: 3, x: null }, e: 4, arr: [9] }
   * )
   ...
   * @returns The value at the specified path, or `undefined` if the path does
   *   not exist
   *
   * @example
   * ```typescript
