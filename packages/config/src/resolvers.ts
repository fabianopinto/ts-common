 ...
 *
 * Supports AWS Systems Manager Parameter Store (`ssm://`) and S3 objects
 * (`s3://`), with retry/backoff via `RetryUtils` and structured logging via
 * `@t68/logger`.
 */
 ...
 *
 * Uses AWS SDK v3 and relies on default AWS credential/region providers
 * available in the process environment. The parameter is fetched with optional
 * decryption.
 *
 ...
 * @param opts.withDecryption - Whether to enable decryption of the parameter
 *   value (default: `true`)
 * @returns The string parameter value
 * @throws `ConfigurationError` when the parameter is not found or has no string
 *   value
 *
 * @example
 * ```typescript
